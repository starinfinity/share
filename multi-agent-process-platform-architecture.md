...
| Open source everything else | LangGraph, FastAPI, React, PostgreSQL + pgvector, Redis, MinIO, Keycloak, Apache Tika, Tesseract, gitpython, docxtpl, Langfuse, Prometheus/Grafana, Argo Workflows. |
...
flowchart TB
    ...
    subgraph API["Backend (FastAPI on K8s)"]
        GW[API Gateway + Auth Keycloak/OIDC]
...
Single-page app with three panes: conversational chat with drag-and-drop multi-file upload (streamed to MinIO via presigned URLs, 200 MB/file cap), a plan-review pane rendering the generated plan as editable task cards with an explicit "Unachievable" section, and a progress pane streaming agent status over WebSocket, ending with a download link for the generated/updated document and the PR URL. Authentication via Keycloak (OIDC); the GitHub identity used for PRs is a service account, with the requesting user recorded in the PR body for traceability.
...
| **Ingestion & Understanding** | Parses every upload into normalized markdown + structured metadata; builds a consolidated "Process Understanding Brief". | Apache Tika for detection; python-pptx / python-docx / openpyxl for native parsing; GPT 5.5 vision for images and embedded diagrams; Tesseract as deterministic OCR fallback for scanned pages. |
...
| **NAS Retrieval (RAG)** | *(existing only)* Retrieves the process's canonical documents from the NAS index; assembles the authoritative process context. | Hybrid search: pgvector cosine (embedding-003) + Postgres full-text (BM25-style); GPT 5.5 prompt-based reranking (no HF cross-encoders). Returns document paths, not just chunks, so the Documentation Agent knows which source document to base the updated version on. |
...
stateDiagram-v2
    ...
    Ingest --> Classify
    Classify --> NASRetrieve: existing process
    Classify --> AskRepo: new process
    NASRetrieve --> RepoDiscover
    RepoDiscover --> Plan
    AskRepo --> Plan: user supplies repo URL
    Plan --> HumanGate: plan + unachievable list
    HumanGate --> Plan: user edits
    HumanGate --> Docs: user approves
...
1. **Ingest & understand.** All uploads parsed into a normalized corpus; GPT 5.5 produces a Process Understanding Brief (process name candidates, intended changes, systems mentioned, open questions). Ambiguities are surfaced in chat immediately rather than silently assumed.
2. **Classify** new vs existing. Low confidence → one clarifying question to the user.
3. **Context enrichment.** Existing: NAS RAG retrieves the canonical process docs, then Repo Discovery maps process → repo (user confirms if below confidence threshold). New: user is asked for the repo URL; the agent validates clone access and default branch before proceeding.
4. **Plan.** Structured plan with ordered tasks (description, rationale, affected files, acceptance criteria, estimated risk) plus the **unachievable list** with concrete reasons per §6.
5. **Human gate.** Approve / edit / reject in the web app. Edits re-run feasibility validation.
6. **Documentation.** New process → tech spec DOCX generated from the official template. Existing → an updated copy of the NAS process document reflecting the approved changes. In both cases the document is delivered as a **download link only** before coding begins; the NAS is never modified, and publishing the updated document remains a human responsibility.
7. **Autonomous coding.** Feature branch; per-task implement → self-check loop; no human interaction until the PR (per your selection). A kill switch in the UI can abort the run at any time.
8. **Validation.** Full test suite + linters + build in the sandbox; failures feed back to the Coding agent with bounded retries. Persistent failures downgrade the task to "attempted — needs human attention" and are reported in the PR body rather than force-merged.
9. **PR.** Branch pushed, `CHANGELOG.md` created/updated, PR opened containing the approved plan, per-task status, test summary, doc link, and requesting user. The platform never merges — merging stays a human action.
